...
bool FRenderer::CreateRayTracingPipeline(FDX12Device* Device)
{
    ...
    RootParameters[2].Constants.Num32BitValues = 13;
...
bool FRenderer::CreateEnvironmentBuildPipelines(FDX12Device* Device)
{
    if (!Device)
    {
        return false;
    ...
    ComPtr<ID3DBlob> ErrorBlob;
    HR_CHECK(D3D12SerializeVersionedRootSignature(&VersionedRootDesc, SerializedSig.GetAddressOf(), ErrorBlob.GetAddressOf()));
    HR_CHECK(Device->GetDevice()->CreateRootSignature(0, SerializedSig->GetBufferPointer(), SerializedSig->GetBufferSize(), IID_PPV_ARGS(EnvironmentBuildRootSignature.ReleaseAndGetAddressOf())));
    ...
    if (!Compiler.CompileFromFile(L"Shaders/EnvEquirectToCubeCS.hlsl", L"CSMain", CSTarget, EquirectBytecode))
    {
    ...
    if (!Compiler.CompileFromFile(L"Shaders/EnvCubeMipGenCS.hlsl", L"CSMain", CSTarget, MipGenBytecode))
    {
    ...
    if (!Compiler.CompileFromFile(L"Shaders/EnvSpecularPrefilterCS.hlsl", L"CSMain", CSTarget, SpecularBytecode))
    {
    ...
    PsoDesc.pRootSignature = EnvironmentBuildRootSignature.Get();

    PsoDesc.CS = { EquirectBytecode.data(), EquirectBytecode.size() };
    HR_CHECK(Device->GetDevice()->CreateComputePipelineState(&PsoDesc, IID_PPV_ARGS(EnvEquirectToCubePipeline.ReleaseAndGetAddressOf())));

    PsoDesc.CS = { MipGenBytecode.data(), MipGenBytecode.size() };
    HR_CHECK(Device->GetDevice()->CreateComputePipelineState(&PsoDesc, IID_PPV_ARGS(EnvCubeMipGenPipeline.ReleaseAndGetAddressOf())));

    PsoDesc.CS = { SpecularBytecode.data(), SpecularBytecode.size() };
    HR_CHECK(Device->GetDevice()->CreateComputePipelineState(&PsoDesc, IID_PPV_ARGS(EnvSpecularPrefilterPipeline.ReleaseAndGetAddressOf())));

    return true;
}


DXGI_FORMAT FRenderer::ResolveEnvironmentBuildFormat() const
{
...
bool FRenderer::BuildEnvironmentFromEquirect(const FRendererConfig& Config)
{
    if (!Device || !TextureLoader)
    {
        return false;
    }

    ComPtr<ID3D12Resource> EquirectTexture;
    ...
    const DXGI_FORMAT EnvironmentBuildFormat = ResolveEnvironmentBuildFormat();
    ...
    ComPtr<ID3D12Resource> RawCube;
    ComPtr<ID3D12Resource> PrefilteredCube;
    ...
    for (uint16_t Mip = 0; Mip < MipLevels; ++Mip)
    {
        ...
        RawUavs[Mip].Texture2DArray.FirstArraySlice = 0;
        RawUavs[Mip].Texture2DArray.ArraySize = 6;
        RawUavs[Mip].Texture2DArray.PlaneSlice = 0;
