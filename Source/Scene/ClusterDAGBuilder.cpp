...
namespace
{
    constexpr uint32_t GVmeshVersion = 26;
    constexpr uint32_t GClusterDAGBuildSemanticVersion = 15;
    ...
    void LogRuntimeClusterHierarchyDiagnostics(const FRuntimeClusterHierarchy& Runtime)
    {
        uint64_t ChildRefTotal = 0;
        uint64_t ParentRefTotal = 0;
        for (const FRuntimeClusterGroup& Group : Runtime.Groups)
        {
            ChildRefTotal += Group.ChildRefCount;
            ParentRefTotal += Group.ParentRefCount;
        }

        uint64_t DrawDataTotal = 0;
        uint64_t TriangleTotal = 0;
        uint32_t MaxClusterMip = 0;
        for (const FRuntimeCluster& Cluster : Runtime.Clusters)
        {
            DrawDataTotal += Cluster.DrawDataCount;
            TriangleTotal += Cluster.TriangleCount;
            MaxClusterMip = (std::max)(MaxClusterMip, Cluster.MipLevel);
        }

        uint64_t IndexTotal = 0;
        for (const FRuntimeClusterDrawData& DrawData : Runtime.DrawDatas)
        {
            IndexTotal += DrawData.IndexCount;
        }

        std::vector<uint32_t> ClustersPerMip(MaxClusterMip + 1u, 0u);
        for (const FRuntimeCluster& Cluster : Runtime.Clusters)
        {
            ++ClustersPerMip[Cluster.MipLevel];
        }

        LogInfo("ClusterDAG runtime hierarchy: groups=" + std::to_string(Runtime.Groups.size())
            + ", clusters=" + std::to_string(Runtime.Clusters.size())
            + ", childRefs=" + std::to_string(Runtime.ChildRefs.size())
            + " (child=" + std::to_string(ChildRefTotal) + ", parent=" + std::to_string(ParentRefTotal) + ")"
            + ", drawDatas=" + std::to_string(Runtime.DrawDatas.size())
            + ", packedIndices=" + std::to_string(Runtime.PackedIndices.size())
            + " (triangles=" + std::to_string(TriangleTotal) + ")");

        std::string MipHistogram;
        for (uint32_t Mip = 0; Mip <= MaxClusterMip; ++Mip)
        {
            MipHistogram += (Mip == 0 ? "" : ", ") + ("mip" + std::to_string(Mip) + "=" + std::to_string(ClustersPerMip[Mip]));
        }
        LogInfo("ClusterDAG runtime clusters per mip: " + MipHistogram);

        const bool bRefsOk = (ChildRefTotal + ParentRefTotal) == Runtime.ChildRefs.size();
        const bool bDrawOk = DrawDataTotal == Runtime.DrawDatas.size();
        const bool bIndexOk = IndexTotal == Runtime.PackedIndices.size();
        const bool bTriOk = (TriangleTotal * 3u) == Runtime.PackedIndices.size();
        if (!bRefsOk || !bDrawOk || !bIndexOk || !bTriOk)
        {
            LogWarning(std::string("ClusterDAG runtime hierarchy invariant mismatch:")
                + (bRefsOk ? "" : " childRefs!=child+parent")
                + (bDrawOk ? "" : " drawDatas!=sum(DrawDataCount)")
                + (bIndexOk ? "" : " packedIndices!=sum(IndexCount)")
                + (bTriOk ? "" : " packedIndices!=3*triangles"));
        }
    }
    ...
    void EncodeRuntimeClusterHierarchy(FClusterDAG& Dag)
    {
        ...
        LogRuntimeClusterHierarchyDiagnostics(Runtime);
    }
    ...
    bool SimplifyGroupGeometryWithAttributes(
        ...
        size_t UniquePositionCount,
        uint32_t BoundaryEdgeCount,
        FBuilderVertexStreams& OutStreams,
        std::vector<uint32_t>& OutIndices,
        ...
        uint32_t& OutPredictedParentCount)
    {
        ...
        const float WedgesPerPosition = UniquePositionCount > 0
            ? static_cast<float>(SourceVertexCount) / static_cast<float>(UniquePositionCount)
            : 0.0f;
        ...
        const float LockedVertexRatio = SourceVertexCount > 0
            ? static_cast<float>(LockedVertexCount) / static_cast<float>(SourceVertexCount)
            : 0.0f;
        ...
        FBuilderVertexStreams BestForceFitStreams;
        std::vector<uint32_t> BestForceFitIndices;
        float BestForceFitResultError = 0.0f;
        uint32_t BestForceFitPredictedParentCount = 0;
        uint32_t BestForceFitTargetClusterTriangles = InitialTargetClusterTriangles;
        size_t BestForceFitTargetIndexCount = 0;
        size_t BestForceFitAttemptCount = 0;
        bool bHasForceFitCandidate = false;
        uint32_t TargetClusterTriangles = InitialTargetClusterTriangles;
        const char* LastFailureReason = "no_attempt";
        bool bReduced = false;
        bool bAcceptedForceFit = false;
        size_t AcceptedForceFitAttemptCount = 0;
        ...
        while (TargetClusterTriangles > 0u)
        {
            ...
            {
                LastFailureReason = "invalid_target_index_count";
                break;
            }
            ...
            {
                LastFailureReason = "empty_simplified_output";
            }
            else if (SimplifiedIndexCount >= SourceIndexCount)
            {
            ...
            else
            {
                ...
                {
                    LastFailureReason = "empty_compacted_output";
                }
                ...
                else
                {
                    ...
                    if (GenerateMeshletsForGeometry(CandidateStreams, CandidateIndices, Params, MeshletBuild))
                    {
                        ...
                        if (bMeaningfulTriangleReduction && bMeaningfulClusterReduction)
                        {
                            const bool bBetterForceFitCandidate =
                                !bHasForceFitCandidate
                                || MeshletBuild.MeshletCount < BestForceFitPredictedParentCount
                                || (MeshletBuild.MeshletCount == BestForceFitPredictedParentCount
                                    && CandidateIndices.size() < BestForceFitIndices.size());
                            if (bBetterForceFitCandidate)
                            {
                                BestForceFitStreams = std::move(CandidateStreams);
                                BestForceFitIndices = std::move(CandidateIndices);
                                BestForceFitResultError = OutResultError;
                                BestForceFitPredictedParentCount = static_cast<uint32_t>(MeshletBuild.MeshletCount);
                                BestForceFitTargetClusterTriangles = TargetClusterTriangles;
                                BestForceFitTargetIndexCount = TargetIndexCount;
                                BestForceFitAttemptCount = AttemptCount;
                                bHasForceFitCandidate = true;
                            }
                        }

                        if (!bMeaningfulTriangleReduction)
                        {
                            LastFailureReason = "insufficient_triangle_reduction";
                        }
                        else if (!bMeaningfulClusterReduction)
                        {
                            LastFailureReason = "insufficient_parent_reduction";
                        }
                        else if (!bSplitSuccess)
                        {
                    ...
                    {
                        LastFailureReason = "meshlet_build_failed";
                    }
                }
            }
        ...
        if (!bReduced && bHasForceFitCandidate)
        {
            OutStreams = std::move(BestForceFitStreams);
            OutIndices = std::move(BestForceFitIndices);
            OutResultError = BestForceFitResultError;
            OutPredictedParentCount = BestForceFitPredictedParentCount;
            FinalTargetClusterTriangles = BestForceFitTargetClusterTriangles;
            FinalTargetIndexCount = BestForceFitTargetIndexCount;
            AcceptedForceFitAttemptCount = BestForceFitAttemptCount;
            bReduced = true;
            bAcceptedForceFit = true;
        }

        if (!bReduced)
        {
            const size_t SimplifiedTriangleCount = LastSimplifiedIndexCount / 3;
            const float BoundaryEdgeRatio = SimplifiedTriangleCount > 0
                ? static_cast<float>(BoundaryEdgeCount) / static_cast<float>(SimplifiedTriangleCount)
                : 0.0f;
            CLUSTER_DAG_LOG_WARNING(PrimitiveIndex, "Level " << Level << " group " << GroupOrdinal << " SimplifyGroupGeometry failed" << ", reason=" << LastFailureReason << ", vertices=" << SourceStreams.Positions.size() << ", triangles=" << SourceTriangleCount << ", wedgesPerPosition=" << FormatFloat(WedgesPerPosition) << ", desiredParents=" << DesiredParentCount << ", targetClusterTriangles=" << FinalTargetClusterTriangles << ", targetTriangles=" << (FinalTargetIndexCount / 3) << ", lockedVertices=" << LockedVertexCount << ", lockedVertexRatio=" << FormatFloat(LockedVertexRatio) << ", boundaryEdges=" << BoundaryEdgeCount << ", boundaryEdgeRatio=" << FormatFloat(BoundaryEdgeRatio) << ", simplifiedTriangles=" << SimplifiedTriangleCount << ", predictedParents=" << LastPredictedParentCount << ", attempts=" << AttemptCount << ", resultError=" << FormatFloat(OutResultError));
            return false;
        }
        ...
        const size_t OutputTriangleCount = OutIndices.size() / 3;
        const float BoundaryEdgeRatio = OutputTriangleCount > 0
            ? static_cast<float>(BoundaryEdgeCount) / static_cast<float>(OutputTriangleCount)
            : 0.0f;
        CLUSTER_DAG_LOG_INFO(PrimitiveIndex, "Level " << Level << " group " << GroupOrdinal << " SimplifyGroupGeometry" << ", srcVertices=" << SourceVertexCount << ", srcTriangles=" << SourceTriangleCount << ", wedgesPerPosition=" << FormatFloat(WedgesPerPosition) << ", outputVertices=" << OutStreams.Positions.size() << ", outputTriangles=" << OutputTriangleCount << ", desiredParents=" << DesiredParentCount << ", predictedParents=" << OutPredictedParentCount << ", forceFit=" << (bAcceptedForceFit ? "true" : "false") << ", forceFitAttempt=" << AcceptedForceFitAttemptCount << ", targetClusterTriangles=" << FinalTargetClusterTriangles << ", targetTriangles=" << (FinalTargetIndexCount / 3) << ", lockedVertices=" << LockedVertexCount << ", lockedVertexRatio=" << FormatFloat(LockedVertexRatio) << ", boundaryEdges=" << BoundaryEdgeCount << ", boundaryEdgeRatio=" << FormatFloat(BoundaryEdgeRatio) << ", attempts=" << AttemptCount << ", resultError=" << FormatFloat(OutResultError) << ", validOutput=" << (bValidOutput ? "true" : "false"));
        return bValidOutput;
    }
        ...
        uint32_t& OutPredictedParentCount)
    {
        ...
        if (BuildMergedClusterGeometry(Dag, Scratch, SourceStreams, SourceIndices, &SourceVertexLocks))
        {
        ...
        {
            CLUSTER_DAG_LOG_WARNING(PrimitiveIndex, "Level " << Level << " group " << GroupOrdinal << " meshopt source emission failed" << ", childClusters=" << ChildClusters.size() << ", fallback=disabled");
        }
        ...
        OutResultError = 0.0f;
        OutPredictedParentCount = 0;
        CLUSTER_DAG_LOG_WARNING(PrimitiveIndex, "Level " << Level << " group " << GroupOrdinal << " group reduction failed" << ", reason=attribute_simplify_failed" << ", fallback=disabled" << ", childClusters=" << ChildClusters.size() << ", sourceTriangles=" << Scratch.ActiveTriangleCount);
        return false;
    }
#endif
    ...
    bool FinalizeRootDAG(
        size_t PrimitiveIndex,
        const char* CompletionReason,
        const std::vector<uint32_t>& CurrentClusters,
        size_t CurrentTriangleCount,
        FBuildState& State,
        FClusterDAG& OutDag)
    {
...
// 5. Feed the new parent clusters back into the next reduction level.
        ...
        for (uint32_t Level = 1; Level < Params.MaxLevels && CurrentClusters.size() > 1; ++Level)
        {
            ...
            for (const std::vector<uint32_t>& ChildClusters : ClusterGroups)
            {
                ...
                if (!BuildMergedClusterScratch(PrimitiveIndex, Level, GroupOrdinal, State.Dag, ChildClusters, LevelEdgeOwners, Scratch))
                {
            ...
            if (bCloseCurrentLevelAsRoot)
            {
                ...
                return FinalizeRootDAG(PrimitiveIndex, RootCloseReason, CurrentClusters, CurrentTriangleCount, State, OutDag);
            }
        ...
        return FinalizeRootDAG(
            PrimitiveIndex,
            CurrentClusters.size() == 1 ? "single_root_converged" : "max_levels_or_convergence_floor",
            CurrentClusters,
...
namespace
{
    ...
    void FillVmeshDagHeader(const FClusterDAG& Dag, const FClusterDAGPackedVertexData& PackedVertexData, FVmeshDagHeader& OutHeader)
    {
        ...
        OutHeader.RuntimePackedIndexCount = static_cast<uint32_t>(Dag.RuntimeHierarchy.PackedIndices.size());
    ...
    {
        if (!WritePodVector(Stream, Dag.RuntimeHierarchy.Groups)
            ...
            || !WritePodVector(Stream, Dag.RuntimeHierarchy.PackedIndices)
    ...
    bool ReadClusterDAGPayload(std::istream& Stream, const FVmeshDagHeader& DagHeader, FClusterDAG& Dag)
    {
        ...
        if (!ReadPodVector(Stream, Dag.RuntimeHierarchy.Groups, DagHeader.RuntimeGroupCount)
            ...
            || !ReadPodVector(Stream, Dag.RuntimeHierarchy.PackedIndices, DagHeader.RuntimePackedIndexCount)
    ...
    bool IsEmptyClusterDAG(const FClusterDAG& Dag)
    {
        return Dag.Clusters.empty()
            ...
            && Dag.RootGroupIndex == GClusterDAGInvalidIndex
            && !Dag.HasRuntimeHierarchy()
            && !Dag.HasPackedVertexData();
    }

    bool ValidateLoadedClusterDAG(const FClusterDAG& Dag)
    {
        return IsEmptyClusterDAG(Dag) || Dag.HasRuntimePayload();
    }
    ...
    void ReleaseClusterDAGAuthoringArrays(FClusterDAG& Dag)
    {
        ...
        if (!Dag.HasRuntimePayload())
        {
            return;
        }

        ReleaseVector(Dag.Clusters);
        ReleaseVector(Dag.Groups);
        ReleaseVector(Dag.Positions);
        ReleaseVector(Dag.Normals);
        ReleaseVector(Dag.UVs);
        ReleaseVector(Dag.Tangents);
        ReleaseVector(Dag.Colors);
        ReleaseVector(Dag.VertexSectionIndices);
        ReleaseVector(Dag.TriangleIndices);
        ReleaseVector(Dag.ExternalEdges);
        ReleaseVector(Dag.ClusterVertices);
        Dag.RootGroupIndex = GClusterDAGInvalidIndex;
    }
    ...
    bool SerializableClusterDAGEqual(FClusterDAG Expected, FClusterDAG Actual)
    {
        ...
        return RuntimeHierarchyEqual(Expected.RuntimeHierarchy, Actual.RuntimeHierarchy)
    ...
    bool BuildVmeshDagPayload(const FClusterDAG& Dag, FVmeshDagPayload& OutPayload)
    {
        ...
        {
            if (!BuildClusterDAGPackedVertexData(Dag, PackedVertexData))
            {
                return false;
            }
        }

        if (!Dag.HasRuntimeHierarchy() || Dag.RuntimeHierarchy.PackedIndices.empty() || !PackedVertexData.IsValid())
        {
            return false;
        }
    ...
    bool BuildVmeshStreamingPagePayload(const FClusterDAG& Dag, uint32_t LocalGroupIndex, std::vector<uint8_t>& OutBytes)
    {
        ...
        PageClusterLocalIndices.reserve(ClusterIndices.size());
        for (uint32_t LocalIndex = 0; LocalIndex < static_cast<uint32_t>(ClusterIndices.size()); ++LocalIndex)
        {
            PageClusterLocalIndices.emplace(ClusterIndices[LocalIndex], LocalIndex);
        }

        for (FRuntimeClusterChildRef& ChildRef : PageChildRefs)
        {
            ...
            const auto LocalIt = PageClusterLocalIndices.find(ChildRef.ClusterIndex);
            if (LocalIt == PageClusterLocalIndices.end())
            {
                return false;
            }
            ChildRef.InstanceIndex = LocalIt->second;
        }
        ...
        for (uint32_t ClusterIndex : ClusterIndices)
        {
            FRuntimeCluster Cluster = Runtime.Clusters[ClusterIndex];
            ...
            Cluster.DrawDataStart = static_cast<uint32_t>(PageDrawDatas.size());
            PageClusters.push_back(Cluster);

            for (uint32_t DrawDataOffset = 0; DrawDataOffset < Cluster.DrawDataCount; ++DrawDataOffset)
            {
                const FRuntimeClusterDrawData& DrawData = Runtime.DrawDatas[Runtime.Clusters[ClusterIndex].DrawDataStart + DrawDataOffset];
...
{
    if (!ClusterDAG.IsValid() && !ClusterDAG.HasRuntimePayload())
    {
        return nullptr;
    }
...
{
    return ClusterDAG.IsValid() || ClusterDAG.HasRuntimePayload();
}

void FMesh::ReleaseClusterDAGAuthoringData()
{
    ReleaseClusterDAGAuthoringArrays(ClusterDAG);
}
