...
#ifndef SSR_HZB_FULL_RES_DEPTH_ENABLED
#define SSR_HZB_FULL_RES_DEPTH_ENABLED 1
#endif
...
VSOutput SsrSWTraceVS(uint VertexId : SV_VertexID)
{
    float2 Positions[3] =
    {
        float2(-1.0f, -1.0f),
...
float4 SsrSWTracePS(VSOutput Input) : SV_Target
{
    if (UseHistory == 0)
    {
        return 0.0f;
