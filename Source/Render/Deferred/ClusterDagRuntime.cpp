...
bool FClusterDagRuntime::ValidatePreparedRuntimeData(const FPreparedData& Data) const
{
    ...
    {
        const FGroupData& Group = Data.Groups[GroupIndex];
    ...
    for (uint32_t RootOrdinal = 0; RootOrdinal < static_cast<uint32_t>(Data.RootGroups.size()); ++RootOrdinal)
    {
        ...
        const FGroupData& RootGroup = Data.Groups[RootGroupIndex];
    ...
    const FRuntimeClusterDrawData& RuntimeDrawData) const
{
    ...
    if (SortedIndex < DrawSections.size())
    {
        ...
        const uint32_t ObjectBaseSectionIndex = SortedIndex - OwnerView.ObjectSectionIndex;
        const uint32_t DrawSectionIndex = ObjectBaseSectionIndex + ObjectSectionIndex;
        if (DrawSectionIndex < DrawSections.size())
        {
    ...
    uint32_t DrawSectionIndex)
{
    ...
    constexpr uint32_t PsoMask =
        RendererUtils::GPipelineKeyDoubleSidedMask | RendererUtils::GPipelineKeyAlphaMaskMask;
    const uint32_t PsoKey = PipelineKey & PsoMask;

    for (uint32_t Index = 0; Index < IndirectDrawRanges.size(); ++Index)
    {
        const FRenderer::FIndirectDrawRange& Existing = IndirectDrawRanges[Index];
        if ((Existing.PipelineKey & PsoMask) == PsoKey)
        {
            return Index;
        }
    }
    ...
    std::vector<uint32_t>& OutLocalDrawDataRangeIndices)
{
    ...
    for (uint32_t LocalDrawDataIndex = 0; LocalDrawDataIndex < static_cast<uint32_t>(RuntimeHierarchy.DrawDatas.size()); ++LocalDrawDataIndex)
    {
        ...
        const uint32_t RangeIndex = GetOrAddRangeForSection(DrawSections, DrawSectionIndex);
        ...
        OutData.DrawDatas.push_back(FDrawData::Make(
            RuntimeDrawData.IndexStart, RuntimeDrawData.IndexCount,
            RangeIndex, 0u, 0u, DrawSectionIndex));
...
void FClusterDagRuntime::InitializeStreamingPageSource(
    const FMeshSection& Section,
    const FRuntimeClusterHierarchy& RuntimeHierarchy,
    ...
    FPreparedData& OutData) const
{
    ...
    PageSource.SetSceneGroupBounds(GroupData.Bounds, GroupData.LodBounds);
...
void FClusterDagRuntime::BuildStreamingPageClusterPayload(
    const FMeshSection& Section,
    uint32_t SortedIndex,
    const FRuntimeClusterHierarchy& RuntimeHierarchy,
    const FRuntimeClusterGroup& RuntimeGroup,
    ...
    FClusterDagStreamingPageSource& PageSource) const
{
    ...
    const uint32_t GroupChildRefStart = RuntimeGroup.ChildRefStart;
    for (uint32_t ChildRefOffset = 0u; ChildRefOffset < RuntimeGroup.ChildRefCount; ++ChildRefOffset)
    {
        const uint32_t ChildClusterIndex = RuntimeHierarchy.ChildRefs[GroupChildRefStart + ChildRefOffset].ClusterIndex;
        if (ChildClusterIndex != GClusterDAGInvalidIndex && ChildClusterIndex < RuntimeHierarchy.Clusters.size())
        {
            PageClusterIndices.push_back(ChildClusterIndex);
        }
    }
    ...
    for (uint32_t LocalClusterIndex : PageClusterIndices)
    {
        ...
        for (uint32_t DrawDataOffset = 0u; DrawDataOffset < RuntimeCluster.DrawDataCount; ++DrawDataOffset)
        {
            ...
            const bool bValidPackedIndexRange = RuntimeDrawData.IndexStart <= PackedIndexCount
                && RuntimeDrawData.IndexCount <= PackedIndexCount - RuntimeDrawData.IndexStart;
            assert(bValidPackedIndexRange && "ClusterDAG runtime draw data index range is out of bounds");
            if (!bValidPackedIndexRange)
            {
                PageSource.bValid = false;
                continue;
            }

            const FClusterDAGPackedVertexData& PackedVertexData = Section.ClusterDagPackedVertexData;
            for (uint32_t IndexOffset = 0u; IndexOffset < RuntimeDrawData.IndexCount; ++IndexOffset)
            {
                ...
                else
                {
                    ...
                    if (!PageSource.AppendPackedVertex(SourceVertexIndex, PackedVertexData))
                    {
                        PageSource.bValid = false;
                        continue;
                    }
    ...
    FPreparedData& OutData)
{
    for (uint32_t LocalGroupIndex = 0; LocalGroupIndex < static_cast<uint32_t>(RuntimeHierarchy.Groups.size()); ++LocalGroupIndex)
    {
        ...
        if (PageIndex != GClusterDagRootStreamingPageIndex)
        {
            ...
            FClusterDagStreamingPageSource& PageSource = OutData.StreamingPageSources[PageIndex];
            BuildStreamingPageClusterPayload(
                Section,
                SortedIndex,
                RuntimeHierarchy,
                RuntimeGroup,
...
bool FClusterDagRuntime::PrepareRuntimeData(FDeferredRenderer& Owner, FPreparedData& OutData)
{
    ...
    for (uint32_t SortedIndex : SortedIndices)
    {
        ...
        const FObject& SectionObject = *DrawSections.GetView(SortedIndex).Object;
        const FRuntimeClusterHierarchy& RuntimeHierarchy = Section.ClusterDagRuntimeHierarchy;
        ...
        const DirectX::XMFLOAT4X4& WorldMatrixRef = SectionObject.GetWorldMatrix();
        const DirectX::XMMATRIX World = DirectX::XMLoadFloat4x4(&WorldMatrixRef);
        const float SectionScale = MatrixMath::ComputeMaxScale(WorldMatrixRef);
    ...
    uint32_t Cursor = 0u;
    for (FRenderer::FIndirectDrawRange& Range : IndirectDrawRanges)
    {
        Range.Start = Cursor;
        Cursor += Range.Count;
    }
    ...
    for (FDrawData& DrawData : OutData.DrawDatas)
    {
        if (DrawData.RangeIndex < IndirectDrawRanges.size())
        {
            const FRenderer::FIndirectDrawRange& Range = IndirectDrawRanges[DrawData.RangeIndex];
            DrawData.RangeCommandStart = Range.Start;
            DrawData.RangeCommandCount = Range.Count;
        }
    }
...
bool FClusterDagRuntime::CreateRuntimeResources(FDeferredRenderer& Owner, FDX12Device* Device, const FPreparedData& Data)
{
    ...
    RuntimeMaxTraversalLevels = 1;
    for (const FGroupData& Group : Data.Groups)
    {
    ...
    ClearAndResize(CandidateClusterEntryBuffers);
    ...
    for (uint32_t FrameIndex = 0; FrameIndex < Frames; ++FrameIndex)
    {
        ...
        CreatePerFrameBuffer(
            FrameIndex,
            L"ClusterDagCandidateClusterEntry",
            CreateRWStructuredBufferDesc<FCandidateClusterEntry>(Data.Clusters.size()),
            CandidateClusterEntryBuffers[FrameIndex],
            true,
            true);
        CreatePerFrameBuffer(
            FrameIndex,
...
void FClusterDagRuntime::AddInitQueuePass(FDeferredPassContext& Context, const char* PassName) const
{
    ...
    Context.Graph.AddPass<FClusterDagInitQueuePassData>(PassName, [this, &Context](FClusterDagInitQueuePassData& Data, FRGPassBuilder& Builder)
    {
        ...
        if (Data.bEnabled)
        {
            ...
            const FRGBufferHandle CandidateQueueHandle = ImportBindlessBuffer(Context.Graph, "ClusterDAG_CandidateClusterEntry", MutableThis.CandidateClusterEntryBuffers[FrameIndex]);
...
void FClusterDagRuntime::DispatchInitQueues(FDeferredRenderer& Owner, FDX12CommandContext& CmdContext, const FCamera& Camera, const char* PassName) const
{
    ...
    const uint32_t RunCountCapacity = static_cast<uint32_t>(IndirectDrawRanges.size());
    const uint32_t DrawDataVisibleEntryCount = RuntimeCommandCount;
    const uint32_t RootGroupCount = VisibleRootCount;
    const uint32_t RuntimeClusterCount = ClusterCount;
    ...
    struct FClusterDagInitBindlessConstants
    {
        ...
        uint32_t CandidateClusterEntryBufferIndex;
        uint32_t VisitedGroupEpochBufferIndex;
        uint32_t RunCountBufferIndex;
        uint32_t RunCountCapacity;
        uint32_t DrawDataVisibleEntryCount;
    ...
    const uint32_t WorkCount = bUseCommittedQueueInit
        ? (std::max)((std::max)(RootGroupCount, RunCountCapacity), DrawDataVisibleEntryCount)
        : (std::max)(
            (std::max)((std::max)(RootGroupCount, RunCountCapacity), RuntimeGroupCount),
            (std::max)(RuntimeClusterCount, DrawDataVisibleEntryCount));
...
void FClusterDagRuntime::DispatchPersistentCull(FDeferredRenderer& Owner, FDX12CommandContext& CmdContext, const char* PassName) const
{
    ...
    const FClusterDagPersistentBindlessConstants BindlessConstants =
    {
        ...
        ForceMipLevel,
        bForceSoftwareRaster ? 1u : 0u,
        SwRasterThresholdPixels
    };
    ...
    const uint32_t WorkCount = (std::max)(RuntimeGroupCount, ClusterCount);
    const uint32_t DispatchCount = (WorkCount + 63u) / 64u;
    CommandList->Dispatch((std::max)(1u, DispatchCount), 1, 1);
}
...
void FClusterDagRuntime::DispatchLevelSplitInit(FDeferredRenderer& Owner, FDX12CommandContext& CmdContext, const FCamera& Camera, const char* PassName) const
{
    ...
    const uint32_t InitWorkCount = (std::max)(
        (std::max)(RuntimeGroupCount, RuntimeClusterCount),
        (std::max)((std::max)(RootGroupCount, RunCountCapacity), DrawDataVisibleEntryCount));
...
void FClusterDagRuntime::DispatchLevelSplitClusterCull(FDeferredRenderer& Owner, FDX12CommandContext& CmdContext, const char* PassName) const
{
    ...
    const FClusterDagLevelSplitClusterCullBindlessConstants ClusterCullConstants =
    {
        ...
        RunCountBuffers[FrameIndex].UavBindlessIndex,
        QueueStateBuffers[FrameIndex].UavBindlessIndex,
        CandidateClusterEntryBuffers[FrameIndex].SrvBindlessIndex,
        ClusterCount,
        DebugStatsIndex,
        VisibleEntryBuffers[FrameIndex].UavBindlessIndex,
        VisibleEntryCounterBuffers[FrameIndex].UavBindlessIndex,
