...
void FDeferredRenderer::RenderFrame(FDX12CommandContext& CmdContext, const D3D12_CPU_DESCRIPTOR_HANDLE& RtvHandle, const FCamera& Camera, float DeltaTime)
{
	...
	if (bUsePathTracing)
	{
		AddPathTracingPass(Graph, Camera, Resources.DepthHandle, Resources.GBufferHandles[0], Resources.GBufferHandles[2], Resources.LightingHandle);
	}
	else
	{
...
void FDeferredRenderer::AddLightingPass(FRenderGraph& Graph, const FDeferredFrameState& FrameState, const std::array<FRGResourceHandle, 3>& GBufferHandles, FRGResourceHandle DepthHandle, FRGResourceHandle GtaoHandle, FRGResourceHandle ShadowHandle, FRGResourceHandle LightingHandle)
{
...
void FDeferredRenderer::AddPathTracingPass(FRenderGraph& Graph, const FCamera& Camera, FRGResourceHandle DepthHandle, FRGResourceHandle GBufferAHandle, FRGResourceHandle GBufferCHandle, FRGResourceHandle LightingHandle)
{
    ...
    Graph.AddPass<FPathTracingPassData>("PathTracing", [&, DepthHandle, GBufferAHandle, GBufferCHandle, LightingHandle](FPathTracingPassData& Data, FRGPassBuilder& Builder)
    {
        if (!bPathTracingEnabled || !bRayTracingPipelineReady || !DepthHandle || !GBufferAHandle || !GBufferCHandle || !LightingHandle)
        {
            return;
        }

        Data.LightingHandle = LightingHandle;
        Data.DepthHandle = DepthHandle;
        Data.GBufferAHandle = GBufferAHandle;
        Data.GBufferCHandle = GBufferCHandle;
        Data.Camera = &Camera;
        Builder.WriteTexture(Data.LightingHandle, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        Builder.ReadTexture(Data.DepthHandle, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    ...
    }, [this, &Graph](const FPathTracingPassData& Data, FDX12CommandContext& CmdContext)
    {
        ...
        ID3D12Resource* LightingTarget = Graph.GetTextureResource(Data.LightingHandle);
        if (!LightingTarget)
        {
            return;
        }
        ...
        LightingUavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
        LightingUavDesc.Format = LightingTarget->GetDesc().Format;
        LightingUavDesc.Texture2D.MipSlice = 0;
        if (RayTracingLightingUavBindlessIndex == UINT32_MAX)
        {
            RayTracingLightingUavBindlessIndex = Device->CreateBindlessUav(LightingTarget, nullptr, LightingUavDesc);
        }
        else if (RayTracingLightingResource != LightingTarget)
        {
            WriteBindlessUav(RayTracingLightingUavBindlessIndex, LightingTarget, nullptr, LightingUavDesc);
        }
        RayTracingLightingResource = LightingTarget;
