...
static const uint SsrHwRayFlags = RAY_FLAG_ACCEPT_FIRST_HIT_AND_END_SEARCH | RAY_FLAG_SKIP_PROCEDURAL_PRIMITIVES;

[numthreads(RayQueryThreadGroupSize, 1, 1)]
void SsrHWTraceCS(uint3 DispatchThreadId : SV_DispatchThreadID)
{
    ...
    RayQuery<SsrHwRayFlags> RayQuery;
    RayQuery.TraceRayInline(Scene, SsrHwRayFlags, 0xFF, Ray);
    ...
    OutputTexture[item.PixelCoord] = float4(color * SsrIntensity, hitWeight);
}
