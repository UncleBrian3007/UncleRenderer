...
FRestirGIPRNG RestirGIPRNGCreate(uint2 Pixel, uint FrameIndex, uint Salt)
{
    ...
    Seed = Hash32(Seed + Pixel.y * 0x85ebca6bu);
    Seed = Hash32(Seed + FrameIndex * 1664525u + Salt);
    Rng.State = Seed;
    return Rng;
}
...
{
    const float CenterLinearDepth = ResolveLinearDepthFromDeviceDepth(CenterDepth, Projection);
    const float SampleLinearDepth = ResolveLinearDepthFromDeviceDepth(SampleDepth, Projection);
...
void CSInitialSampling(uint3 DispatchThreadId : SV_DispatchThreadID)
{
    ...
    const float3 WorldPos = ReconstructWorldPosition(FullPos, Depth, uint2(FullWidth, FullHeight), ViewProjectionInverse);
    float2 Xi = 0.0f.xx;
...
void CSSpatialResampling(uint3 DispatchThreadId : SV_DispatchThreadID)
{
    ...
    const float CenterLinearDepth = ResolveLinearDepthFromDeviceDepth(CenterDepth, Projection);
    const float3 CenterWorldPos = ReconstructWorldPosition(CenterFullPos, CenterDepth, uint2(FullWidth, FullHeight), ViewProjectionInverse);
    ...
    for (uint Iteration = 0u; Iteration < MaxIterations; ++Iteration)
    {
        ...
        const float3 NeighborWorldPos = ReconstructWorldPosition(NeighborFullPos, NeighborDepth, uint2(FullWidth, FullHeight), ViewProjectionInverse);
